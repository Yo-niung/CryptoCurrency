...
public class API {
    ...
    public static Map<UUID, Double> getPlayerBalances(String crypto) {
        return MySql.getPlayerBalances(crypto);
    }
    public static boolean showTopPlayers(CommandSender sender, String cryptoType) {
        Map<UUID, Double> balances = getPlayerBalances(cryptoType);
        ...
        sender.sendMessage(Message.getMessage(UUID.randomUUID(), "message_top_players").replace("{crypto}", cryptoType.toUpperCase()));
        int count = 0;
        for (Map.Entry<UUID, Double> entry : sortedBalances) {
            ...
            if (playerName == null) continue; // 이름이 null인 경우 생략
            double balance = entry.getValue();
            sender.sendMessage(Message.chat("&e" + (count + 1) + ". " + playerName + " - " + API.moneyFormatter.format(balance) + " " + cryptoType.toUpperCase()));
            count++;
        }
        return true;
    }
    ...
    public static int buyCrypto(String player, String crypto, double amount) {
        ...
        double total_price = money_price + fee;
        if (player_balance < total_price) return 11;
        if (CryptoCurrency.conn != null) {
            MySql.setPlayerBalance(UUID, player, API.getFormatter(crypto).format(balance + amount), crypto);
            CryptoCurrency.getEconomy().withdrawPlayer(player, total_price);
            Settings.cryptos.get(crypto).supply += amount;
            return 10;
        }
        Settings.cryptos.get(crypto).wallet.set(UUID, balance + amount);
    ...
    public static int sellCrypto(String player, String crypto, double amount) {
        ...
        if (CryptoCurrency.conn != null) {
            MySql.setPlayerBalance(UUID, player, API.getFormatter(crypto).format(balance - amount), crypto);
            CryptoCurrency.getEconomy().depositPlayer(player, total_price);
            Settings.cryptos.get(crypto).supply -= amount;
            return 10;
        }
        Settings.cryptos.get(crypto).wallet.set(UUID, balance - amount);
